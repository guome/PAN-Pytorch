...
from utils import runningScore, cal_text_score, cal_kernel_score, cal_recall_precision_f1
...
class Trainer(BaseTrainer):
    def __init__(self, config, model, criterion, train_loader, weights_init=None):
        super(Trainer, self).__init__(config, model, criterion, train_loader, weights_init)
    ...
    def _eval(self):
        ...
        img_path = os.path.join(self.test_path, 'images')
        gt_path = os.path.join(self.test_path, 'gt_labels')
        ...
        for img_path in tqdm(img_paths, desc='test models'):
            ...
            #np.savetxt(save_name, boxes_list.reshape(-1, 8), delimiter=',', fmt='%d', encoding='utf-8')

        # 开始计算 recall precision f1
        result_dict = cal_recall_precision_f1(gt_path=gt_path, result_path=result_save_path, text_type='curve')
    ...
    def _on_epoch_finish(self):
        ...
        if self.config['trainer']['metrics'] == 'hmean':  # 使用f1作为最优模型指标
            ...
            net_save_path = '{}/PANNet_epoch_{}_hmean_{:.4f}.pth'.format(self.checkpoint_dir, self.epoch_result['epoch'], hmean)
            ...
            net_save_path = '{}/PANNet_epoch_{}_loss_{:.4f}.pth'.format(self.checkpoint_dir, self.epoch_result['epoch'], self.epoch_result['train_loss'])
