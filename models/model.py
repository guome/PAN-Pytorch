...
norm_layer_dict = {'normal': nn.BatchNorm2d, 'sync_bn': SynchronizedBatchNorm2d}
...
class Model(nn.Module):
    def __init__(self, model_config: dict):
        ...
        norm_layer = norm_layer_dict.get(norm_layer, None)
        model_config.pop('norm_layer')
        backbone_model, backbone_out = backbone_dict[backbone]['models'], backbone_dict[backbone]['out']
        self.backbone = backbone_model(pretrained=pretrained, norm_layer=norm_layer)
        self.segmentation_head = segmentation_head_dict[segmentation_head](backbone_out, **model_config, norm_layer=norm_layer)
