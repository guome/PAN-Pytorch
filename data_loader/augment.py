...
def show_pic(img, bboxes=None, name='pic'):
    ...
    show_img = img.copy()
    for box in bboxes:
        num_points = box.shape[0]
        for idx in range(num_points-1):
           cv2.line(show_img, tuple(box[idx].astype(np.int)), tuple(box[idx+1].astype(np.int)), (255, 0, 0), 2)
        cv2.line(show_img, tuple(box[num_points-1].astype(np.int)), tuple(box[0].astype(np.int)), (255, 0, 0), 2)
...
class DataAugment():
    ...
    def random_scale(self, im: np.ndarray, text_polys: list, scales: np.ndarray or list) -> tuple:
        """
        ...
        :return: 经过缩放的图片和文本
        """
        tmp_text_polys = copy.deepcopy(text_polys)
        rd_scale = float(np.random.choice(scales))
        im = cv2.resize(im, dsize=None, fx=rd_scale, fy=rd_scale)
        for idx, poly in enumerate(tmp_text_polys):
            poly = poly * rd_scale
            tmp_text_polys[idx] = poly
    ...
    def random_rotate_img_bbox(self, img, text_polys, degrees: numbers.Number or list or tuple or np.ndarray,
        ...
        for bbox in text_polys:
            rot_bbox = []
            for point in bbox:
                rot_point = np.dot(rot_mat, np.array([point[0], point[1], 1]))
                rot_bbox.append(rot_point)
            rot_bbox = np.array(rot_bbox)
            rot_text_polys.append(rot_bbox)
    ...
    def resize(self, im: np.ndarray, text_polys: list,
    ...
    def horizontal_flip(self, im: np.ndarray, text_polys: list) -> tuple:
        """
        对图片和文本框进行水平翻转
        :param im: 图片
        :param text_polys: 文本框
        ...
        """
        flip_text_polys = copy.deepcopy(text_polys)
        flip_im = cv2.flip(im, 1)
        h, w, _ = flip_im.shape
        for idx, poly in enumerate(flip_text_polys):
            poly[:, 0] = w - poly[:, 0]
            flip_text_polys[idx] = poly
        return flip_im, flip_text_polys

    def vertical_flip(self, im: np.ndarray, text_polys: list) -> tuple:
        """
         对图片和文本框进行竖直翻转
        :param im: 图片
        :param text_polys: 文本框
    ...
    def test(self, im: np.ndarray, text_polys: list):
        print('随机尺度缩放')
