...
from utils import order_points_clockwise_list
...
class ImageDataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            import traceback
            traceback.print_exc()
            print('invalid image {}, error [{}]'.format(img_path, e))
            return self[index+1]
    ...
    def load_data(self, data_list: list) -> list:
        ...
        for img_path, label_path in data_list:
            ...
            bboxs, text_tags = self._get_icdar2019_annotation(label_path)
            if len(bboxs) > 0:
    ...
    def _get_annotation(self, label_path: str) -> tuple:
        ...
        with open(label_path, encoding='utf-8', mode='r') as f:
            for line in f.readlines():
                ...
                try:
                    box = order_points_clockwise_list(np.array(list(map(float, params[:8]))).reshape(4, 2))
                    if cv2.arcLength(box.astype(np.int), True) > 0:
                        boxes.append(box)
                        label = params[-1]
                        if label == '*' or label == '###':
                    ...
                    print('load label failed on {}, error [{}]'.format(label_path, e))
                    exit(0)
        return boxes, text_tags

    def _get_icdar2019_annotation(self, label_path: str) -> tuple:
        boxes = []
        text_tags = []
        with open(label_path, encoding='utf-8', mode='r') as f:
            for line in f.readlines():
                ...
                try:
                    length = len(params) - 1
                    assert length % 2 == 0, 'point num can not divided by 2'
                    box = np.array(list(map(float, params[:length]))).reshape(-1, 2)
                    if cv2.arcLength(box.astype(np.int), True) > 0:
                        boxes.append(box)
                        label = params[-1]
                        if label == '*' or label == '###':
    ...
    def _get_ctw1500_annotation(self, label_path: str) -> tuple:
        boxes = []
        text_tags = []
        with open(label_path, encoding='utf-8', mode='r') as f:
            for line in f.readlines():
                ...
                try:
                    ...
                    box = np.asarray(np.asarray(box) + ([x1 * 1.0, y1 * 1.0] * 14)).reshape(-1, 2)
                    if cv2.arcLength(box.astype(np.int), True) > 0:
                        boxes.append(box)
                        text_tags.append(True)
                except Exception as e:
    ...
    def __init__(self, dataset_list: list, ratio_list: list, module_args: dict,
        ...
        for _dataset, batch_ratio_d in zip(dataset_list, ratio_list):
            ...
            _data_loader = DataLoader(dataset=_dataset,
                                      ...
                                      drop_last=module_args['loader']['drop_last'],
...
if __name__ == '__main__':
    ...
    train_data = ImageDataset(
        data_list=[
            (r'D:/Datasets/CTW1500/train/text_image/0010.jpg', 'D:/Datasets/CTW1500/train/text_label_curve/0010.txt')],
        input_size=640,
        img_channel=3,
        shrink_ratio=0.5,
